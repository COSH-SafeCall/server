...
모델/API/voice는 발행된 promptRelease/personaPrompt에서 읽는다. 단, 세션 setup에서 일관된 남성 음성을 보장하기 위해 `FATHER`는 `Algieba`, `FRIEND`는 `Rasalgethi`로 고정하고 `MOTHER`는 발행된 `voiceId`를 사용한다. 환경변수만 입력하거나 DRAFT 초안을 넣는 것으로 통화가 준비되지는 않는다. [프롬프트 발행](#프롬프트-발행)을 참고한다.
