...
이 문서는 기존 인증·사용자·홈·통화 코드를 API 1~4장의 28개 HTTP 작업에 맞춘 당시의 변경 기록이다. 당시 제외한 5~7장은 이후 구현하여 현재 총 33개 HTTP 작업을 제공한다. 현재 범위와 실행 진입점은 [문서 목록](README.md), 최종 실행 수량·시각은 [검증 결과](verification.md)에서 관리한다.
