...
키 생성 복구 기록에는 업무용 Hikari 풀과 별도로 최대 2개의 DB 연결을 사용한다. 같은 DB 접속·인증·세션 초기화 설정을 복사하며 별도 환경 변수는 필요 없다. DB 연결 예산은 인스턴스당 업무 풀 최대치에 2개를 더해 산정한다. 이 전용 풀은 짧은 복구 기록 INSERT/커밋에만 사용하며 키 저장소 호출과 폐기 워커에는 사용하지 않는다.
