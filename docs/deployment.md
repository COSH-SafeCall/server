...
local/prod를 함께 활성화하면 실패한다. 환경변수별 의미는 [환경 설정](setup.md)에서 관리한다. `.env.example`은 빈 양식이며 배포용 credential 파일이 아니다.

## 배포 전에 완성할 항목

| 항목 | 현재 구현 | 남은 작업 |
|---|---|---|
| 키 저장 | DB에는 keyRef, local에는 사용자/임시 키 | 외부 저장소, 모든 서버의 동일 참조 해석, 32바이트 키 반환 계약, 이미 폐기된 키에도 성공하는 discard, 폐기 재시도·관측 |
...
| Gemini | 단기 token·연결 제약·종료 처리 | 출시 모델/API/voice/instruction 검수 자료와 설정 대조 |
| 안심 메시지 | M01 작성 자료·자격 재검증, 미설정 지도 템플릿은 null | Naver 템플릿·좌표 순서·HTTPS 호스트·지원 브라우저 검수 후 MESSAGE_MAP_* 등록. 실제 작성 화면·위치 취득 검수 |
| ACCOUNT 삭제 | 로컬 삭제+LOCAL_DELETED, 별도 암호문과 선점 기한, 카카오 연결 해제 재시도·R03 조회 | 서버 전용 KAKAO_ADMIN_KEY, 실제 앱 연결 해제·다중 worker 장애·외부 키 폐기 검수 |
| 복구·관측 | 정리 작업 및 안전한 오류 응답 | 상태 확인, 여러 서버의 작업 경쟁, DB·외부 키 백업 복원과 삭제 기록 재적용 검수 |
| 운영 사건 O01 | 허용목록·배치 원자성·세션 중복 제거·신규 분당 120건·14일 보존 | 배포 WEB_VERSION, 브라우저 사건 계측과 지연 목표 확인. 프록시/APM의 금지 본문 수집 차단 |

커밋 결과 불명 상태에서는 키를 보존하고 DB 결과를 먼저 확인한다. 로컬 삭제 후 외부 정리 오류를 FAILED로 표시하지 않는다. 백업 복원 뒤 폐기한 키나 삭제 데이터를 다시 서비스하지 않도록 복원 절차에 반영한다.

외부 키 저장소의 `reference(allocationId)`는 키 생성 없이 안정적인 참조를 반환해야 하며 `create(allocationId)`의 결과도 같아야 한다. 새 할당에는 새 참조를 사용한다. 미완료 `keyDiscardJob`을 백업·복원에 포함하고, 과거 버전에서 이미 참조를 잃은 키는 별도 대조한다.
