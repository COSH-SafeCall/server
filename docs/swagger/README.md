...
화면 그룹은 `01. 인증·세션`부터 `11. 이용 기록·데이터 삭제`까지 고정한다. 그룹 안은 API 번호순이며 인증만 A05를 맨 위에 둔다. 번호는 찾아보기 순서이고 실제 테스트는 아래 흐름을 따른다. 통화 생성 직후 C05 유지, 회원 온보딩 중 프로필·연락망 확인은 해당 그룹 설명을 참고한다.

| 순서 | 시나리오 | 완료 기준 |
|---|---|---|
| 1 | [인증·온보딩](auth.md) | 게스트 또는 회원의 A06 step=COMPLETE |
...
게스트는 회원 프로필·메시지·이력·삭제를 건너뛴다. 온보딩 MESSAGE_TEST에서 M01을 먼저 확인할 때는 TEST 모드를 사용한다. 기능을 차단하는 동의 철회·계정 삭제는 다른 테스트를 마친 테스트 계정에서 진행한다.
...
| `Idempotency-Key` | A07, U02/U05/U06/U08/U09/U10/U12, C01/C04/C06/C07, R02에 UUID. 새 작업은 새 키, 같은 작업 재시도는 같은 키·본문 |
...
| 401 | 쿠키 수용·만료, A05 상태 |
| 403 | Origin·CSRF, 회원·동의·온보딩·재인증 조건 |
| 404 | ID 및 세션·페이지 소유권 |
