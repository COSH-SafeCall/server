...
class HttpGeminiClientTest {
	...
	private GeminiClient.IssueRequest request() {
		return new GeminiClient.IssueRequest("models/gemini-3.1-flash-live-preview","v1beta","Alnilam","synthetic system policy",
	...
	@Test void grantLocksModelVoiceAndInstructionsWithoutResumption() {
		server.expect(requestTo(URL)).andExpect(method(HttpMethod.POST)).andExpect(header("x-goog-api-key","synthetic-key"))
			...
			.andExpect(jsonPath("$.bidiGenerateContentSetup.generationConfig.speechConfig.voiceConfig.prebuiltVoiceConfig.voiceName").value("Alnilam"))
