...
public class MessageRepository {
	...
	public record Material(UUID userId, String keyRef, byte[] name, byte[] phone, boolean isConfirmed,
		Instant sessionExpiresAt, boolean isLocationGranted,
	...
	public List<Material> snapshot(UUID sessionId, Instant now) {
		// READ_COMMITTED의 단일 SELECT로 권한·프로필·연락망·통화를 같은 스냅샷에서 읽는다.
		return jdbc.query("""
			SELECT u.`id` AS `userId`, u.`keyRef`, u.`nameCipher`, u.`phoneCipher`,
				u.`profileConfirmedAt` IS NOT NULL AS `isConfirmed`, s.`expiresAt`,
				us.`locationPermission`='GRANTED' AS `isLocationGranted`,
				(u.`status`='DELETION_PENDING' OR EXISTS(SELECT 1 FROM `deletionJob` j
					WHERE j.`userId`=u.`id` AND j.`pendingMarker`=1 AND j.`scope`='ACCOUNT')) AS `isCleanupPending`,
			...
			FROM `webSession` s JOIN `appUser` u ON u.`id`=s.`userId` JOIN `userSetting` us ON us.`userId`=u.`id`
			LEFT JOIN `emergencyContact` r ON r.`userId`=u.`id`
			WHERE s.`id`=? AND s.`kind`='MEMBER' AND s.`status`='ACTIVE' AND s.`expiresAt`>?
			ORDER BY r.`slot`
			""", (r,n) -> {
				...
				return new Material(uuid(r,"userId"), r.getString("keyRef"), r.getBytes("nameCipher"), r.getBytes("phoneCipher"),
					...
					r.getBoolean("isLocationGranted"), r.getBoolean("isCleanupPending"),
