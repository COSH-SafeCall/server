...
@Check(name = "ckWebSessionEntity", constraints = "octet_length(`sessionHash`)=32 AND octet_length(`csrfHash`)=32 AND `kind` IN ('ANONYMOUS','GUEST','KAKAO') AND ((`kind`='KAKAO')=(`userId` IS NOT NULL)) AND `status` IN ('ACTIVE','REVOKED','EXPIRED') AND ((`status`='REVOKED')=(`revokedAt` IS NOT NULL)) AND `expiresAt`>`createdAt` AND (`kind`<>'GUEST' OR `expiresAt`<=DATE_ADD(`createdAt`, INTERVAL 24 HOUR)) AND (`sensitiveVerifiedAt` IS NULL OR `kind`='KAKAO') AND (`sensitiveVerifiedAt` IS NULL OR (`sensitiveVerifiedAt`>=`createdAt` AND `sensitiveVerifiedAt`<`expiresAt`))")
@Entity
...
public class WebSessionEntity {
	...
	@Column(name = "sensitiveVerifiedAt", columnDefinition = "DATETIME(6)")
	private Instant sensitiveVerifiedAt;
