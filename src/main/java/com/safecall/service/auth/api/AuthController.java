...
public class AuthController {
	...
	public SessionView session(HttpServletRequest req,HttpServletResponse res){
		String cookie=WebCookies.read(req),encoded=WebCookies.readProfilePrefill(req);var result=auth.bootstrap(cookie);WebCookies.set(res,result,clock.instant());
		if(encoded!=null)WebCookies.clearProfilePrefill(res);var prefill=result.cookie()==null&&result.view().isAuthenticated()?oauth.consumeProfilePrefill(cookie,encoded):null;return result.view().withProfilePrefill(prefill);
	}
	...
	@PostMapping(value="/auth/kakao/authorization",consumes="application/json") @Operation(operationId="A02",summary="카카오 OAuth 시작",description="서비스 동의는 로그인 이후 U05에서 기록합니다. 요청은 purpose만 받습니다.")
	...
	public ResponseEntity<Void> callback(HttpServletRequest req,HttpServletResponse res,@RequestParam String state,@RequestParam(required=false) String code,@RequestParam(required=false) String error){
		var result=oauth.callback(WebCookies.read(req),state,code,error,req.getHeader("User-Agent"));if(result.session()!=null)WebCookies.set(res,result.session(),clock.instant());WebCookies.setProfilePrefill(res,result.profilePrefill());
