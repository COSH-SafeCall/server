...
public record CallPolicy(String version,int leaseSeconds,int issueTimeoutSeconds,
	int minuteLimit,int guestDailyLimit,int memberDailyLimit,String validatedModel,String validationRef,int modelMaxSeconds) {
	public CallPolicy(@Value("${app.call.policy-version:mvp-2026-09-11}") String version,
		...
		@Value("${app.call.member-daily-limit:100}") int memberDailyLimit,
		...
		if(version.isBlank()||version.length()>32||leaseSeconds<=5||issueTimeoutSeconds<1||issueTimeoutSeconds>=leaseSeconds
			||minuteLimit<1||guestDailyLimit<1||memberDailyLimit<1||modelMaxSeconds<0)throw new IllegalStateException("Invalid call policy.");
		this.version=version;this.leaseSeconds=leaseSeconds;this.issueTimeoutSeconds=issueTimeoutSeconds;
		this.minuteLimit=minuteLimit;this.guestDailyLimit=guestDailyLimit;this.memberDailyLimit=memberDailyLimit;
